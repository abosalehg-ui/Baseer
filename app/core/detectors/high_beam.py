...
class HighBeamDetector(BaseViolationDetector):
    ...
    def _run(
        ...
        # 1) خريطة (frame_no → bright vehicle track_ids)
        bright_per_frame: dict[int, set[int]] = {}
        ...
        for track in tracks:
            ...
            for det in track.detections:
                if det.frame_no % self._sample_every_n_frames != 0:
                    continue
                sampled_frames.add(det.frame_no)

        # نقرأ كل إطار مسحوب مرة واحدة
        ...
        for track in tracks:
            ...
            for det in track.detections:
                if det.frame_no % self._sample_every_n_frames != 0:
                    continue
                frame = frame_cache.get(det.frame_no)
                if frame is None:
                    continue
                if not self._is_night_frame(frame, cv2):
                    continue
                if self._has_glare_in_bbox(frame, det.bbox, cv2):
                    bright_per_frame.setdefault(det.frame_no, set()).add(track.track_id)

        # 2) تجميع لكل track: تتابع الإطارات الساطعة
        violations: list[ViolationCandidate] = []
        for track in tracks:
            if track.class_name not in VEHICLE_CLASSES:
                continue
            track_frames = sorted(f for f, ids in bright_per_frame.items() if track.track_id in ids)
            if not track_frames:
                continue
    ...
    def _is_night_frame(self, frame: np.ndarray, cv2) -> bool:
    ...
    def _has_glare_in_bbox(
        self, frame: np.ndarray, bbox: tuple[float, float, float, float], cv2
    ) -> bool:
